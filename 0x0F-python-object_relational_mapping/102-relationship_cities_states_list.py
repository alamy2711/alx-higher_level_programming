#!/usr/bin/python3
"""
Lists all City objects from the 'hbtn_0e_101_usa' database.
"""
import sys
from relationship_state import Base, State
from relationship_city import City
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker


if __name__ == '__main__':
    """
    Establishes a connection to the database and retrieves
    all City objects from the 'hbtn_0e_101_usa' database.
    """

    # Create the database URI using the provided credentials
    db_uri = 'mysql+mysqldb://{}:{}@localhost:3306/{}'.format(
        sys.argv[1], sys.argv[2], sys.argv[3])

    # Create an SQLAlchemy engine and create tables if not exist
    engine = create_engine(db_uri, pool_pre_ping=True)

    # Create a session class with the engine
    Session = sessionmaker(bind=engine)

    # Create a session instance
    session = Session()

    # Query and retrieve all City objects joined with their corresponding State
    cities = session.query(State).join(City).order_by(City.id).all()

    # Print the City information with the associated State
    for state in cities:
        for city in state.cities:
            print("{}: {} -> {}".format(city.id, city.name, state.name))
